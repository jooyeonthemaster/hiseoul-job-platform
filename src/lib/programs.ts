...
export const PORTFOLIO_PROGRAMS: PortfolioProgram[] = [
  {
    ...
    // 풀네임(name)은 과정 선택 카드 등에 노출. 칩·목록 제목용 짧은 표시명은 '전문가 양성과정'까지만
    // 유지해 정식 과정명 느낌을 살리되 UI 가 길어지지 않게 한다.
    shortName: '생성형 AI 디지털 마케팅 전문가 양성과정',
    // '생성형 AI 디지털 마케팅'은 과거 표기(구 shortName)라 매칭용 별칭으로 보존한다.
    aliases: ['생성형 AI 디지털 마케팅', 'AI 마케팅 역량 강화 교육', '영상콘텐츠 마케터', '영상콘텐츠 마케터 양성과정'],
    courseType: 'domestic',
    audience: '내국인 청년',
    hours: '300시간',
